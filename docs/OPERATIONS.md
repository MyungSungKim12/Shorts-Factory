...
제작 전 자동 모드로 복귀하려면 카드의 **수동 예약 취소**를 사용한다. 의도된 계약은 마감 전에만 취소를 허용하고 해당 회차를 자동 제작·업로드 경로로 되돌리는 것이다. 다만 현재 릴리스 후보에는 취소 후 DB 행과 카드가 `manual/cancelled`로 남아 업로드 결정이 `hold`가 되는 결함이 확인되어 수정 대기 중이다. 수정 배포 전에는 다음을 필수 확인한다.

- 취소 응답 뒤 새로고침한 카드가 `mode=auto`, `state=auto`인지 확인한다.
- `manual/cancelled`가 남으면 자동 복귀가 완료되지 않은 것이므로 해당 회차를 자동으로 실행하지 말고 후속 수정 또는 운영자 판단을 기다린다.
...
- 실패·반려·검수 대기·보류 수동 회차는 승인 없이 자동 대체하거나 업로드하지 않는다. 더 진행하지 않으려면 **건너뛰기**로 해당 수동 회차를 종료한다.
...
4. 영상 길이: 본문 목표 53~75초, 제목·CTA 포함 최종 상한 180초. 최종 60초 미만·180초 초과는 차단.
