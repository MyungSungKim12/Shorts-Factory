...
| 자동 실행 | cron, 캐시 워밍 1회 + 사전 제작 4회 + 업로드 4회 (KST) |
...
| 회차 | 소재 입력 마감·사전 제작(KST) | 업로드(KST) | 카테고리 |
|------|-------------------------------|-------------|----------|
| 1 | 09:00 | 11:00 | 과학의 경계/미해결 관측 |
| 2 | 12:00 | 14:00 | 숨겨진 세계/금지된 구조 |
| 3 | 15:00 | 17:00 | 과학의 경계/미해결 관측 |
| 4 | 19:00 | 21:00 | 숨겨진 세계/금지된 구조 |
...
1. 서버 `.env`에 설정된 `DASHBOARD_TOKEN`을 대시보드 상단 **관리자 토큰** 칸에 입력한다. 프론트엔드는 이 값을 현재 브라우저 탭의 `sessionStorage`에만 저장하며 서버 로그나 영구 저장소에는 기록하지 않는다.
2. 오늘 또는 내일 회차를 선택하고 소재, 강조점, 포함·제외 조건, 참고 링크를 입력한 뒤 **소재 확인**을 누른다. `needs_input`이면 해석 후보를 선택해 다시 확인하고, `reservable`이면 검증 방식과 미디어 가능성을 검토한다.
3. **이 소재 예약**을 눌러 `reserved`를 확인한다. 입력과 예약 변경은 표의 사전 제작 시각 직전까지만 가능하며, 마감부터는 제작 작업자가 회차를 잠근다.
4. 사전 제작이 끝나면 `review_ready` 영상과 제목·설명·태그·QC 결과를 검토한다. 이 상태에서는 승인 전 YouTube 업로드가 일어나지 않는다.
5. 업로드 시각 전에 승인하면 `approved`로 대기하고 정시 cron이 업로드한다. 정시까지 승인하지 않으면 `held`로 보류하며 자동 영상으로 대체하지 않는다. `held` 또는 업로드 시각이 지난 `review_ready`를 승인하면 API가 즉시 업로드 작업을 한 번 시작한다.
6. 반려할 때는 사유를 입력한다. 활성 `data/work/{run_id}` 산출물은 `data/rejected/{run_id}-attempt-{N}`으로 이동하고 `REJECTED_RETENTION_DAYS`(기본 7일) 후 정리된다. 이벤트와 반려 사유는 장애 분석을 위해 남는다.

제작 전 자동 모드로 복귀하려면 카드의 **수동 예약 취소**를 사용한다. 마감 전에만 취소할 수 있으며, 완료 후 해당 회차는 자동 제작·업로드 경로로 돌아간다. 다음을 확인한다.

- 취소 응답 뒤 새로고침한 카드가 `mode=auto`, `state=auto`인지 확인한다.
- 카드와 API 응답이 `mode=auto`, `state=auto`인지 확인한다.
- 제작이 `locked` 이후로 진행됐거나 입력 마감이 지난 회차는 취소하지 않는다.

수동 소재 확인이 실패하거나 해당 소재를 더 사용하지 않으려면 **자동 소재로 전환**을 사용한다. 활성 작업자가 없는 수동 게이트를 제거하며 시각별 동작은 다음과 같다.

- 제작 시작 전: 기존 사전 제작 cron과 업로드 cron을 그대로 기다린다.
- 제작 시작 후·업로드 전: 해당 회차 자동 영상을 즉시 사전 제작하고 기존 업로드 cron이 정시에 올린다.
- 업로드 시각 후: 해당 회차 자동 제작·업로드 파이프라인을 즉시 한 번 실행한다.
- 이미 검토·승인·업로드 단계이거나 활성 작업자가 있는 회차는 자동 전환할 수 없다.

### 실패·재시도 운영

- `failed` 또는 `rejected`이고 활성 작업자/전역 잠금이 없을 때만 재시도하거나 건너뛸 수 있다.
- **같은 소재 재시도**는 저장된 검증 결과를 재사용해 새 attempt를 `reserved`로 만든다. **새 소재로 재검사**는 저장된 입력으로 검사부터 다시 시작하며, 마감 전에는 반려 카드에서 완전히 다른 소재를 입력할 수도 있다.
- 각 재시도는 attempt 번호를 올리고 이전 반려 산출물을 덮어쓰지 않는다. 반려 보관본과 이벤트 이력으로 시도별 원인을 추적한다.
- 실패·반려 수동 회차는 임의로 자동 대체하지 않는다. 운영자가 **자동 소재로 전환**을 누른 경우에만 기존 자동 경로로 복구하며, 업로드하지 않으려면 **건너뛰기**로 종료한다. 검수 대기·보류 회차는 승인 전 업로드하지 않는다.
- 업로드 실패는 상태를 `failed`로 돌려 작업자 점유를 해제한다. 원인을 확인한 뒤 재시도하고, 이미 업로드된 `video_id`가 있는지 먼저 확인해 중복 업로드를 막는다.
...
4. 영상 길이: 새 대본은 본문 60~68초·최종 70~85초를 목표로 하며, 제목·CTA 포함 최종 상한은 90초다. 콘텐츠 최소 길이는 두지 않는다.
...
| `TTS_SPEAKING_RATE` | TTS 요청 속도. Chirp 3 HD는 공급자 제한으로 1.0 사용 |
| `NARRATION_TARGET_LUFS` | 제목·본문·CTA 나레이션 정규화 목표(기본 -16 LUFS) |
...
| 업로드 안 됨 `허용 범위 위반` | 영상이 손상된 0초 파일이거나 90초 초과. TTS 길이 과다 → 대본 축약 |
| `제목 지시문이 실제 제목에 포함됨` | 모델이 JSON 작성 예시를 복사함. 작가가 한 번 재작성하고 계속 실패하면 검증 템플릿 사용 |
| 품질검사 `dark_content` | 본문 영역이 전체 25% 초과 또는 6초 이상 연속 저휘도. 다른 소스 또는 제한 밝기 보정 후 재제작 |
| 품질검사 `audio_loudness_range` | 최종 음량 범위가 10 LU 초과. 제목·본문·CTA 개별 정규화 단계와 TTS 공급자 확인 |
...
| 영상 길이 오류 | 과거 검증 상한과 현재 운영 기준 불일치 | 콘텐츠 최소 길이는 두지 않고 최종 90초 상한을 일관 적용 |
...
5. 원복 시 백업 코드를 복원하고 cron은 직전 4회 운영(11:00·14:00·17:00·21:00)으로 되돌린다. DB·준비본은 백업본과 현재 상태를 비교한 뒤 복원한다.
