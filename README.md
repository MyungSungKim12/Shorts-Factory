...
- 11시 기묘한 자연 현상, 14시 미해결 과학 관측, 17시 숨겨진 세계와 구조, 21시 역사 미스터리로 회차를 구분한다.
...
제작이 시작되기 전 자동 운영으로 되돌리려면 **수동 예약 취소**를 사용한다. 취소 API가 수동 예약을 해제하고 해당 회차를 자동 경로로 돌리는 것이 운영 계약이지만, 현재 릴리스 후보에서는 취소 행이 `manual/cancelled`로 남아 업로드 경로를 보류하는 문제가 확인되어 수정 대기 중이다. 수정 배포 전에는 취소 후 카드가 `auto`인지 반드시 확인하고, `manual/cancelled`이면 해당 회차를 자동 운영으로 간주하지 않는다.
