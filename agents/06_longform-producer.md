...
8. 스타일은 제작 전에 PNG 미리보기로 확인하고 `documentary`, `cinematic`, `clean_news` 중 하나를 선택한다.
...
10. 6분 롱폼은 최소 20개 화면 장면으로 구성하고, 그중 최소 15개는 영상 소스 또는 AI 영상 소스를 우선 사용한다.
11. 정지 이미지는 지도·문서·근거 사진처럼 필요한 경우에만 사용하고, 흔들리는 줌·패닝으로 억지 영상화하지 않는다.
