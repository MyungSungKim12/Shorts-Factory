...
업로드 이력과 공개 조회수를 모아 회차·카테고리별 결과를 요약한다. 별도 성과 수집기가 저장한 조회수·좋아요·공유·구독 전환·평균 시청률 데이터는 리서처의 성과 기반 소재 피드백에 사용된다.
...
성과 스냅샷은 다음 리서처 프롬프트에 참고 패턴으로 주입된다. 단, 성과 데이터가 소재를 자동 확정하지는 않으며, 리서처는 같은 장소·대상·사건 반복을 피하면서 상위 소재의 구조만 활용한다. 표본이 8개 미만인 그룹은 소재 성과 차이로 확정하지 않는다.
